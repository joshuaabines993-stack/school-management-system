...
import { 
  ...
  Info, Download, Menu, X, Receipt, Calendar, ArrowRight, Edit3, Camera, Save
} from 'lucide-react';
...
const StudentAccounting = () => {
  ...
  // Profile Edit States (Gayang-gaya sa Dashboard)
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-slate-50 font-black animate-pulse text-slate-400 uppercase tracking-widest">Loading Finance Records...</div>;
    ...
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden font-sans">
      ...
      >
        ...
        <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
          ...
          <SidebarBtn icon={<CreditCard size={18}/>} label="Accounting" active />
        </nav>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto relative flex flex-col">
        ...
        <nav className="sticky top-0 z-30 bg-white border-b border-slate-200 px-6 py-3 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <h2 className="font-black text-slate-800 text-sm uppercase tracking-widest hidden sm:block">Accounting Portal</h2>
          </div>
        ...
        <div className="max-w-6xl mx-auto p-6 md:p-12 w-full space-y-8">
          <header className="animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              <span className="bg-yellow-500 text-[#001f3f] px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest shadow-md italic">S.Y. {studentData?.school_year || '2025-2026'}</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter mb-2">
              Accounting <span style={{ color: branding.theme_color }}>Records</span>
            </h1>
            <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.3em]">Official Assessment for Student ID: {studentData?.student_id}</p>
          </header>
            ...
            <div className="space-y-8">
               <div className={`p-8 rounded-[2.5rem] border-4 ${isLocked ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100'}`}>
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       <p className={`font-black text-xl leading-none ${isLocked ? 'text-red-700' : 'text-emerald-700'}`}>FINANCE</p>
      ...
      {/* EDIT PROFILE MODAL - GINAYA SA DASHBOARD */}
      {isEditModalOpen && (
